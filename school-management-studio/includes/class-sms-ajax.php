<?php
/**
 * Generic AJAX CRUD for the studio.
 *
...
class SMS_Ajax {
    ...
    public function register() {
        $entities = array( 'student','staff','class_item','subject','fee_structure','invoice','book',
            'route','vehicle','hostel','room','session_item','category','medium','student_type',
            'role','admission','inquiry','notice','event','meeting','homework_item','study_material',
            'lecture','ticket','exam_item','exam_group','income_item','expense_item','concession_type',
            'school' );

        foreach ( array( 'save', 'delete' ) as $op ) {
            foreach ( $entities as $e ) {
                add_action( "wp_ajax_sms_{$op}_{$e}", array( $this, $op . '_entity' ) );
            }
        }
        add_action( 'wp_ajax_sms_save_settings', array( $this, 'save_settings' ) );
        add_action( 'wp_ajax_sms_quick_search',  array( $this, 'quick_search' ) );
    }
    ...
    public function save_entity() {
        ...
        $entity = preg_replace( '/^sms_save_/', '', $action );
    ...
    public function delete_entity() {
        ...
        $action = isset( $_POST['action'] ) ? sanitize_text_field( wp_unslash( $_POST['action'] ) ) : '';
        $entity = preg_replace( '/^sms_delete_/', '', $action );
        $table  = $this->table_for( $entity );
        if ( ! $table ) wp_send_json_error( 'bad entity' );
        ...
        global $wpdb;
        $wpdb->delete( $wpdb->prefix . 'sms_' . $table, array( 'id' => $id ) );
    ...
    public function save_settings() {
        ...
        update_option( 'sms_settings', array_merge( get_option( 'sms_settings', array() ), $clean ) );
        wp_send_json_success();
    }
    ...
    private function log( $module, $action, $id = 0 ) {
        global $wpdb;
        $wpdb->insert( $wpdb->prefix . 'sms_logs', array(
